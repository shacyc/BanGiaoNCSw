...
--hs BO TUC THCS
,nvl(couNTpupil_BOHOCTHCS.slhs,0) TOTALPUPILNUDT_BOHOCTHCS
,nvl(couNTpupil_BOHOCTHPT.slhs,0) TOTALPUPILNUDT_BOHOCTHPT
,nvl(couNTpupil_BOHOCTHPT_10.slhs,0) TOTALPUPILNUDT_BOHOCTHPT_10
,nvl(couNTpupil_BOHOCTHPT_11.slhs,0) TOTALPUPILNUDT_BOHOCTHPT_11
,nvl(couNTpupil_BOHOCTHPT_12.slhs,0) TOTALPUPILNUDT_BOHOCTHPT_12
...
JOIN (
  ...
  FROM saMS.edu_pupil_studytimeline eps
    ...
    AND epc.wholeyearconductrankid = 1
    AND epc.isdeleted = 0
...
JOIN (
  ...
  FROM saMS.edu_pupil_studytimeline eps
    ...
    AND epc.schoolid = eps.schoolid
    AND epc.schoolclassid = eps.schoolclassid
    AND epc.wholeyearconductrankid = 2
    AND epc.isdeleted = 0
    AND epc.schoolyearid = eps.schoolyearid
  WHERE eps.schoolyearid = 21
  AND eps.schooltermid = 2 AND EPS.LINKSCHOOLID IS NULL
...
JOIN (
  ...
  FROM saMS.edu_pupil_studytimeline eps
    ...
    AND epc.schoolid = eps.schoolid
    AND epc.schoolclassid = eps.schoolclassid
    AND epc.wholeyearconductrankid = 3
    AND epc.isdeleted = 0
    AND epc.schoolyearid = eps.schoolyearid
  WHERE eps.schoolyearid = 21
  AND eps.schooltermid = 2
  AND currentpupilstatusid = 1 AND EPS.LINKSCHOOLID IS NULL
  AND eps.isdeleted = 0
  AND eps.isactive = 1
  GROUP BY EPS.schoolid
  )couNTpupil_HKTB
...
JOIN (
  ...
  FROM saMS.edu_pupil_studytimeline eps
    ...
    AND epc.schoolid = eps.schoolid
    AND epc.schoolclassid = eps.schoolclassid
    AND epc.wholeyearconductrankid = 4
    AND epc.isdeleted = 0
    AND epc.schoolyearid = eps.schoolyearid
  WHERE eps.schoolyearid = 21
  AND eps.schooltermid = 2
...
JOIN (
  ...
  FROM saMS.edu_pupil_studytimeline eps
    ...
    AND epc.isdeleted = 0
    AND epc.schoolyearid = eps.schoolyearid
  WHERE eps.schoolyearid = 21 AND EPS.LINKSCHOOLID IS NULL
  AND eps.schooltermid = 2
  AND eps.currentpupilstatusid = 1
  AND eps.isdeleted = 0
  AND eps.isactive = 1
...
JOIN (
  ...
  WHERE eps.schoolyearid = 21
  AND eps.schooltermid = 2
  AND eps.currentpupilstatusid = 1 AND EPS.LINKSCHOOLID IS NULL
  AND eps.isdeleted = 0
  AND eps.isactive = 1
  GROUP BY EPS.schoolid
...
JOIN (
  ...
  AND eps.isdeleted = 0 AND EPS.LINKSCHOOLID IS NULL
  AND eps.isactive = 1
  AND eps.currentpupilstatusid IN (4,10,13,12,18)
  GROUP BY EPS.schoolid
  )couNTpupil_BOHOCTHCS
 ON es.schoolid = couNTpupil_BOHOCTHCS.schoolid

--ENDREGION

--REGION TONG SO HS BO HOC THPT
...
JOIN (
  ...
  AND eps.currentpupilstatusid IN (4,10,13,12,18)
  GROUP BY EPS.schoolid
  )couNTpupil_BOHOCTHPT
 ON es.schoolid = couNTpupil_BOHOCTHPT.schoolid

--ENDREGION

--REGION TONG SO HS BO HOC 10,11,12
...
JOIN (
  ...
  AND eps.currentpupilstatusid IN (4,10,13,12,18)
  GROUP BY EPS.schoolid
  )couNTpupil_BOHOCTHPT_10
 ON es.schoolid = couNTpupil_BOHOCTHPT_10.schoolid
...
JOIN (
  ...
  AND eps.currentpupilstatusid IN (4,10,13,12,18)
  GROUP BY EPS.schoolid
  )couNTpupil_BOHOCTHPT_11
 ON es.schoolid = couNTpupil_BOHOCTHPT_11.schoolid
    LEFT 
JOIN (
  SELECT eps.schoolid, COUNT(DISTINCT eps.pupilid)slhs 
  FROM saMS.edu_pupil_studytimeline eps
  ...
  AND eps.currentpupilstatusid IN (4,10,13,12,18)
  GROUP BY EPS.schoolid
  )couNTpupil_BOHOCTHPT_12
 ON es.schoolid = couNTpupil_BOHOCTHPT_12.schoolid
--ENDREGION

  
WHERE es.isactive = 1 AND es.isdeleted = 0

  and ES.schoolid = 10031798
