...
--      AND eps.schoolyearid = 2 AND eps.schooltermid = 2
   ...
   AND es1.isdeleted = 0
    WHERE ES.ISDELETED = 0 AND ES.ISACTIVE = 1
          ...
          AND to_char(ep.birthday,'YYYY') = 2013
          AND ep.currentpupilstatusid = 1
          AND NOT EXISTS
            (
              SELECT *
                FROM sams.edu_pupil_studytimeline eps1
                WHERE eps1.schoolyearid = 21 AND eps1.schooltermid = 1
                AND eps1.isactive = 1 AND eps1.isdeleted = 0
                AND ep.pupilid = eps1.pupilid 
                AND ep.currentpupilstatusid = 1
            )
