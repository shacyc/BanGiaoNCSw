  ...
  FROM sams.edu_pupil ep
   ...
   AND es1.isdeleted = 0
    WHERE ES.ISDELETED = 0 AND ES.ISACTIVE = 1
          ...
          AND ep.fatherphone IS NULL AND ep.motherphone IS NULL
          AND ep.guardianphone IS NULL AND ep.contactphone IS NULL AND ep.econtactbookphonenumber IS NULL
       ...
       GROUP BY MO1.organid, MO1.ORGANNAME,es.schoolname,ec.classname,esn.schoolclassname,ep.pupilid,ep.fullname
      ORDER BY MO1.ORGANNAME,es.schoolname,ep.fullname
